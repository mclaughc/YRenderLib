...
#include "YRenderLib/PixelFormat.h"
...
enum GPU_RESOURCE_TYPE
{
    ...
    GPU_RESOURCE_TYPE_INPUT_LAYOUT,
...
enum SHADER_PROGRAM_BYTECODE_TYPE
{
    SHADER_PROGRAM_BYTECODE_TYPE_D3D_SM40,
    SHADER_PROGRAM_BYTECODE_TYPE_D3D_SM41,
    SHADER_PROGRAM_BYTECODE_TYPE_D3D_SM50,
    SHADER_PROGRAM_BYTECODE_TYPE_GLSL_120,
    SHADER_PROGRAM_BYTECODE_TYPE_GLSL_330,
    SHADER_PROGRAM_BYTECODE_TYPE_GLSL_450,
    SHADER_PROGRAM_BYTECODE_TYPE_GLSL_ES_100,
    SHADER_PROGRAM_BYTECODE_TYPE_GLSL_ES_300,
    NUM_SHADER_PROGRAM_BYTECODE_TYPES
};
...
// Struture representing a RGBA colour
struct FloatColor
{
    FloatColor() {}
    FloatColor(const float* color);
...
struct RENDERER_RASTERIZER_STATE_DESC
{
    ...
    RENDERER_RASTERIZER_STATE_DESC(RENDERER_FILL_MODE fillMode, RENDERER_CULL_MODE cullMode, bool frontCounterClockwise, bool scissorEnable, int32 depthBias, float slopeScaledDepthBias, bool depthClipEnable)
        : FillMode(fillMode), CullMode(cullMode), FrontCounterClockwise(frontCounterClockwise), ScissorEnable(scissorEnable), DepthBias(depthBias), SlopeScaledDepthBias(slopeScaledDepthBias), DepthClipEnable(depthClipEnable) {}
...
struct RENDERER_FACE_STENCIL_OP
{
    ...
    RENDERER_FACE_STENCIL_OP() {}

    RENDERER_FACE_STENCIL_OP(RENDERER_STENCIL_OP failOp, RENDERER_STENCIL_OP depthFailOp, RENDERER_STENCIL_OP passOp, GPU_COMPARISON_FUNC compareFunc)
        : FailOp(failOp), DepthFailOp(depthFailOp), PassOp(passOp), CompareFunc(compareFunc) {}
};
...
struct RENDERER_DEPTHSTENCIL_STATE_DESC
{
    ...
    RENDERER_DEPTHSTENCIL_STATE_DESC(bool depthTestEnable, bool depthWriteEnable, GPU_COMPARISON_FUNC depthFunc)
        : DepthTestEnable(depthTestEnable), DepthWriteEnable(depthWriteEnable), DepthFunc(depthFunc)
        , StencilTestEnable(false), StencilReadMask(0xFF), StencilWriteMask(0xFF)
        , StencilFrontFace(RENDERER_STENCIL_OP_KEEP, RENDERER_STENCIL_OP_KEEP, RENDERER_STENCIL_OP_KEEP, GPU_COMPARISON_FUNC_ALWAYS)
        , StencilBackFace(RENDERER_STENCIL_OP_KEEP, RENDERER_STENCIL_OP_KEEP, RENDERER_STENCIL_OP_KEEP, GPU_COMPARISON_FUNC_ALWAYS) {}

    RENDERER_DEPTHSTENCIL_STATE_DESC(bool depthTestEnable, bool depthWriteEnable, GPU_COMPARISON_FUNC depthFunc, bool stencilTestEnable, uint8 stencilReadMask, uint8 stencilWriteMask, const RENDERER_FACE_STENCIL_OP& stencilFrontFace, const RENDERER_FACE_STENCIL_OP& stencilBackFace)
        : DepthTestEnable(depthTestEnable), DepthWriteEnable(depthWriteEnable), DepthFunc(depthFunc)
        , StencilTestEnable(stencilTestEnable), StencilReadMask(stencilReadMask), StencilWriteMask(stencilWriteMask)
        , StencilFrontFace(stencilFrontFace), StencilBackFace(stencilBackFace) {}
...
struct RENDERER_BLEND_STATE_DESC
{
    ...
    RENDERER_BLEND_STATE_DESC() {}

    RENDERER_BLEND_STATE_DESC(bool blendEnable, RENDERER_BLEND_OPTION srcBlend, RENDERER_BLEND_OP blendOp, RENDERER_BLEND_OPTION destBlend, RENDERER_BLEND_OPTION srcBlendAlpha, RENDERER_BLEND_OP blendOpAlpha, RENDERER_BLEND_OPTION destBlendAlpha, bool colorWriteEnable)
        : BlendEnable(blendEnable), SrcBlend(srcBlend), BlendOp(blendOp), DestBlend(destBlend), SrcBlendAlpha(srcBlendAlpha), BlendOpAlpha(blendOpAlpha), DestBlendAlpha(destBlendAlpha), ColorWriteEnable(colorWriteEnable) {}
...
struct GPU_RENDER_TARGET_VIEW_DESC
{
    ...
    GPU_RENDER_TARGET_VIEW_DESC(GPUTextureCube *pTexture, uint32 mipLevel, CUBEMAP_FACE faceIndex);
    GPU_RENDER_TARGET_VIEW_DESC(GPUTextureCubeArray *pTexture, uint32 mipLevel, uint32 arrayIndex);
    GPU_RENDER_TARGET_VIEW_DESC(GPUTextureCubeArray *pTexture, uint32 mipLevel, uint32 arrayIndex, CUBEMAP_FACE faceIndex);
...
struct GPU_DEPTH_STENCIL_BUFFER_VIEW_DESC
{
    ...
    GPU_DEPTH_STENCIL_BUFFER_VIEW_DESC(GPUTextureCube *pTexture, uint32 mipLevel, CUBEMAP_FACE faceIndex);
    GPU_DEPTH_STENCIL_BUFFER_VIEW_DESC(GPUTextureCubeArray *pTexture, uint32 mipLevel, uint32 arrayIndex);
    GPU_DEPTH_STENCIL_BUFFER_VIEW_DESC(GPUTextureCubeArray *pTexture, uint32 mipLevel, uint32 arrayIndex, CUBEMAP_FACE faceIndex);
