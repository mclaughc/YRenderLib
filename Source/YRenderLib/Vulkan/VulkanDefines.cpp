...
void LogVulkanResult(const char* channelName, const char* functionName, LOGLEVEL level, VkResult res, const char* msg, ...)
{
    ...
    va_start(ap, msg);
    SmallString tempMsg;
    tempMsg.FormatVA(msg, ap);
    va_end(ap);

    tempMsg.AppendFormattedString("%s (%d)", VkResultToString(res), static_cast<int32>(res));

    Log::GetInstance().Write(channelName, functionName, level, tempMsg);
}
